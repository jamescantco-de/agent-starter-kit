 ...
 *
 * Features:
 *   - X Article: Full long-form essay from PRODUCTION_SHIELD.md + banner_production_shield.jpg
...
const PUBLISHED_STATE_FILE = path.join(REPO_ROOT, "published_production_shield.json");
const BANNER_FILE = path.join(REPO_ROOT, "assets", "banner_production_shield.jpg");
const ARTICLE_MD_FILE = path.join(REPO_ROOT, "PRODUCTION_SHIELD.md");
...
export const X_ARTICLE_TITLE = "The Production Shield: Why 90% of Vibe-Coded Apps Die in Production (And How to Harden Them)";
...
export const BSKY_THREAD = [
  {
    "num": 1,
    "hasMedia": true,
    "text": "Anyone with an AI coding agent can build an impressive prototype on localhost in an afternoon.\n\nThen you point real users at it, and 90% of projects crumble.\n\nHere are the 4 SuperSkills of \"The Production Shield\" that turn fragile AI code into enterprise-grade software \ud83e\uddf5\ud83d\udc47"
  },
  {
    "num": 2,
    "hasMedia": false,
    "text": "Wall 1: Silent Security Holes.\n\nAI models write code that works, not code that is secure. They routinely create endpoints with zero tenant isolation, leaking user data.\n\nOur security-audit SuperSkill runs a 4-phase threat model before every deploy, catching IDOR and injection automatically."
  },
  {
    "num": 3,
    "hasMedia": false,
    "text": "Wall 2: Phantom Regressions (AI Groundhog Day).\n\nYou ask the agent to fix checkout; it quietly breaks discount codes.\n\nOur quality-engineering SuperSkill blocks the agent from editing code until it writes a failing reproduction test. No fix is complete without a passing test suite."
  },
  {
    "num": 4,
    "hasMedia": false,
    "text": "Walls 3 & 4: Database Crawls and Sluggish Frontends.\n\n\u2022 database-ops: Automatically indexes foreign keys and enforces zero-downtime expand-contract migrations.\n\u2022 performance-review: Audits Core Web Vitals, eliminates cascading React re-renders, and keeps bundles lean."
  },
  {
    "num": 5,
    "hasMedia": false,
...
async function uploadBannerToX() {
  ...
  form.append("media", blob, "banner_production_shield.jpg");
...
function auditAll() {
  console.log(`\n======================================================`);
  console.log(`  AUDITING: X ARTICLE (PRODUCTION_SHIELD.md)`);
  console.log(`======================================================`);
  ...
  console.log(`Cover:       assets/banner_production_shield.jpg`);
  ...
  BSKY_THREAD.forEach((item) => {
    ...
    if (item.hasMedia) console.log(`   [Attached Media: assets/banner_production_shield.jpg]`);
    console.log(`---`);
