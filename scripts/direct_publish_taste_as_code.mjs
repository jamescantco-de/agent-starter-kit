 ...
 *
 * Features:
 *   - X Article: Full long-form essay from TASTE_AS_CODE.md + banner_taste_as_code.jpg
...
const PUBLISHED_STATE_FILE = path.join(REPO_ROOT, "published_taste_as_code.json");
const BANNER_FILE = path.join(REPO_ROOT, "assets", "banner_taste_as_code.jpg");
const ARTICLE_MD_FILE = path.join(REPO_ROOT, "TASTE_AS_CODE.md");
...
export const X_ARTICLE_TITLE = "Taste as Code: Why Most AI Apps Look Cheap (And How to Force Agents to Have Taste)";
...
export const BSKY_THREAD = [
  {
    "num": 1,
    "hasMedia": true,
    "text": "You can spot a vibe-coded app from a mile away:\nGeneric purple gradient buttons, flat cards with no depth, and jittery CSS transitions.\n\nFrontier LLMs have no innate taste.\n\nHere is how we translated Apple HIG and design engineering into deterministic rules: \"Taste as Code\" \ud83e\uddf5\ud83d\udc47"
  },
  {
    "num": 2,
    "hasMedia": false,
    "text": "Rule 1: The 60-30-10 Rule (No More Colour Salad).\n\nAgents love giving every tag, icon, and button a random colour.\n\nOur design-polish SuperSkill enforces:\n\u2022 60% Dominant canvas\n\u2022 30% Structural cards & subtle borders\n\u2022 10% High-contrast accent strictly for CTAs and active states."
  },
  {
    "num": 3,
    "hasMedia": false,
    "text": "Rule 2: The 44px Law & Physical Compression.\n\nTouch targets must be at least 44x44px, even for tiny icons.\n\nEvery button must include active:scale-[0.98] with subtle rim glow. When pressed, it physically compresses like a hardware switch.\n\nTaste is physical feedback."
  },
  {
    "num": 4,
    "hasMedia": false,
    "text": "Rule 3: Physics Over Easing (Killing Linear CSS).\n\nEase-in-out transitions feel like PowerPoint. Real objects have mass, tension, and friction.\n\nOur motion-engineering SuperSkill enforces standardised spring physics tokens, with automatic reduced-motion accessibility fallbacks."
  },
  {
    "num": 5,
    "hasMedia": false,
...
async function uploadBannerToX() {
  ...
  form.append("media", blob, "banner_taste_as_code.jpg");
...
function auditAll() {
  console.log(`\n======================================================`);
  console.log(`  AUDITING: X ARTICLE (TASTE_AS_CODE.md)`);
  console.log(`======================================================`);
  ...
  console.log(`Cover:       assets/banner_taste_as_code.jpg`);
  ...
  BSKY_THREAD.forEach((item) => {
    ...
    if (item.hasMedia) console.log(`   [Attached Media: assets/banner_taste_as_code.jpg]`);
    console.log(`---`);
