#!/usr/bin/env node
/**
 * Direct Multi-Platform Publisher (X Article & Bluesky Thread)
 *
 * Fully native Node.js script — zero external npm dependencies.
 * Bypasses OmniSocials completely for 100% direct API control.
 *
 * Features:
 *   - X Article: Full long-form essay from OVERVIEW.md + banner cover photo
 *   - Bluesky: 5-post native thread + banner embed + clickable link facets
 *   - Idempotency: Guards against accidental double-posting via published.json
 *
 * Usage:
 *   node scripts/direct_publish.mjs --dry-run
 *   node scripts/direct_publish.mjs --verify-auth
 *   node scripts/direct_publish.mjs --create-draft-x
 *   node scripts/direct_publish.mjs --publish [--force]
 */
...
const PUBLISHED_STATE_FILE = path.join(REPO_ROOT, "published.json");

// Load .env.local manually (or fallback to process.env in CI)
function loadEnv() {
...
export const X_ARTICLE_TITLE = "1.13 Billion Tokens Burned... And Counting: Lessons from a Non-Coder Building with AI Agents";

export function parseOverviewToBlocks() {
  const mdPath = path.join(REPO_ROOT, "OVERVIEW.md");
...
I’m a Dad in commercial tech, but over the last year, my machine has burned 1.13 billion tokens building real software with AI agents.

Here are the 3 real lessons I learnt after hitting every wall imaginable 🧵👇`
  },
  {
    num: 2,
    hasMedia: false,
    text: `Wall #1 is the Context Sinkhole.

You run a test or check git, and the terminal dumps 3,000 lines of boilerplate into the prompt.

Suddenly 80% of your context window is gone, and the model starts forgetting instructions from 10 minutes ago.

Context is your scarcest asset.`
  },
  {
    num: 3,
    hasMedia: false,
    text: `The fix was the 3-Tier Hierarchy & The 1-2-3 Rule:

• Tier 1: Brains (Frontier LLMs for architecture)
• Tier 2: Skills (Procedural SOPs in markdown)
• Tier 3: Scripts (0-token local scripts for mechanical work)

3rd time doing a task in a week? Automate it into a script.`
  },
  {
    num: 4,
    hasMedia: false,
    text: `On the CLI, we run RTK (Rust Token Killer) as a proxy.

It intercepts git, diff, and test commands, stripping out terminal noise before it hits the LLM.

It saved over 18.2M tokens on dev operations alone on my machine - operating at 97%+ efficiency.`
  },
  {
    num: 5,
    hasMedia: false,
    text: `Everything I use - master AGENTS.md, symlink configs, self-healing correction rules, and starter scripts - is now open-source and MIT-licensed.

Free for anyone building with agents. Grab it here:
...
async function uploadBannerToX() {
  const creds = getXCreds();
  const filePath = path.join(REPO_ROOT, "assets", "banner.jpg");
  const fileBytes = fs.readFileSync(filePath);
  const blob = new Blob([fileBytes], { type: "image/jpeg" });
  const form = new FormData();
  form.append("media", blob, "banner.jpg");
...
async function createXArticleDraft(mediaId) {
  const creds = getXCreds();
  const { blocks } = parseOverviewToBlocks();
  ...
  for (const item of BSKY_THREAD) {
    ...
    if (item.hasMedia && blob) {
      record.embed = {
        ...
        images: [
          {
            alt: "Agent Starter Kit - 1.13 Billion Tokens Burned... And Counting",
            image: blob
          }
        ]
      };
...
function auditAll() {
  console.log(`\n======================================================`);
  console.log(`  AUDITING: X ARTICLE (OVERVIEW.md)`);
  console.log(`======================================================`);
  const { blocks, rawContent } = parseOverviewToBlocks();
  ...
  console.log(`Cover:       assets/banner.jpg`);
  ...
  BSKY_THREAD.forEach((item) => {
    ...
    if (item.hasMedia) console.log(`   [Attached Media: assets/banner.jpg]`);
    console.log(`---`);
...
async function main() {
  ...
  // Check draft-id override if passed
  ...
  if (!isDryRun && !isVerifyAuth && !isCreateDraft && !isPublish) {
    console.log("Usage: node scripts/direct_publish.mjs [--dry-run | --verify-auth | --create-draft-x | --publish] [--force]");
    process.exit(0);
  }
  ...
  if (isVerifyAuth || isCreateDraft || isPublish) {
    ...
    if (!bskyAuth.success) {
      console.error("❌ Bluesky Auth Failed:", bskyAuth.error);
      process.exit(1);
    }
  ...
  if (isPublish) {
    ...
    const replyText = `Here is the open-source repository:\nhttps://github.com/jamescantco-de/agent-starter-kit\n\nAll scripts, rules, and instruction frameworks are MIT-licensed. Star or fork it if you find it useful!`;
