 ...
 *
 * Features:
 *   - X Article: Full long-form essay from SUPERSKILLS.md + banner_superskills.jpg
...
const PUBLISHED_STATE_FILE = path.join(REPO_ROOT, "published_superskills.json");
const BANNER_FILE = path.join(REPO_ROOT, "assets", "banner_superskills.jpg");
const ARTICLE_MD_FILE = path.join(REPO_ROOT, "SUPERSKILLS.md");
...
export const X_ARTICLE_TITLE = "The 40-Year-Old Design Rule That Fixed My AI Agent's Memory (And Why 'More Skills' Is A Trap)";
...
export const BSKY_THREAD = [
  {
    num: 1,
    hasMedia: true,
    text: `The instinct with AI coding agents is always "more skills".

Add 50 skills, 100 skills, 180 skills... until you realise your agent silently drops 40 of them on boot to fit context limits.

Here is how a 40-year-old software design rule fixed my agent's memory 🧵👇`
  },
  {
    num: 2,
    hasMedia: false,
    text: `The trap is called Attention Dilution ("Lost in the Middle").

When an agent evaluates 100 candidate tools, its accuracy collapses. It confuses overlapping tools and forgets your base rules.

Worse, you become a micromanager again — remembering obscure tool names instead of delegating outcomes.`
  },
  {
    num: 3,
    hasMedia: false,
    text: `The solution is Progressive Disclosure (Jakob Nielsen & Apple HIG, 1990s):

Show only what is needed for the primary task upfront. Defer deep manuals until requested.

Think macOS print dialogue: Cmd + P shows 3 controls, not postscript colour profiles. The power is there, but hidden.`
  },
  {
    num: 4,
    hasMedia: false,
    text: `For AI agents, this creates "SuperSkills":

• Level 1: Registry (1-line description on boot, ~30 tokens)
• Level 2: Router (master workflow & decision tree, <150 lines)
• Level 3: Deep References (checklists lazy-loaded only when triggered)

~180 micro-skills collapsed cleanly into 12 SuperSkills.`
  },
  {
    num: 5,
    hasMedia: false,
    text: `Now, I never type "run evm-decimals then contract-math".

I say "run a security audit on this PR". The agent handles the multi-phase battery in one pass.

All 12 SuperSkills are open-source on GitHub:
...
async function uploadBannerToX() {
  ...
  form.append("media", blob, "banner_superskills.jpg");
...
function auditAll() {
  console.log(`\n======================================================`);
  console.log(`  AUDITING: X ARTICLE (SUPERSKILLS.md)`);
  console.log(`======================================================`);
  ...
  console.log(`Cover:       assets/banner_superskills.jpg`);
  ...
  BSKY_THREAD.forEach((item) => {
    ...
    if (item.hasMedia) console.log(`   [Attached Media: assets/banner_superskills.jpg]`);
    console.log(`---`);
...
async function main() {
  ...
  if (isPublish) {
    ...
    console.log(`• Recorded to:         published_superskills.json`);
    console.log("======================================================");
  }
}
